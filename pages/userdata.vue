<template>
    <div>
      <v-dialog v-model="editDialog" class="responsive-dialog" width="500px">
      ...
      >
        <template v-slot:top>
          <v-toolbar style="background-color: #abd1c6" flat>
            ...
            <v-dialog v-model="dialogDelete" width="350px">
              <v-card>
                <v-card-title class="text-h5" style="background-color: #abd1c6;">Are you sure want to delete this data?</v-card-title>
...
@media (max-width: 600px) {
  .responsive-dialog .v-dialog__content {
    ...
    methods: {
      ...
      async saveEditDialog() {
        try {
          ...
          this.message = "Data successfully edited"
          this.openDialog = true
      ...
      async addCard() {
        try {
          ...
          await this.$api.post('/user/add_card/' + this.editItem.id)
            .then((res) => {
              this.message = "Card successfully added"
            })
            .catch((error) => {
